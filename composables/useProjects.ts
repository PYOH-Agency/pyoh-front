  ...
  const fetchProjects = async (options?: {
    ...
    try {
      ...
      const response = await find<ProjectsResponse>('projects', {
        populate: options?.populate || ['media', 'homeMedia', 'coverPicture', 'project_types', 'url_videos.url', 'url_videos.name'],
  ...
  const fetchFeaturedProjects = async () => {
    try {
      ...
      // Essayer avec $fetch directement
              const url = `${strapiUrl}/api/projects?populate[0]=coverPicture&populate[1]=media&populate[2]=homeMedia&populate[3]=project_types&populate[4]=url_videos.url&populate[5]=url_videos.name&filters[featured][$eq]=true&sort=createdAt:desc&pagination[pageSize]=10`
